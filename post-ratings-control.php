...
<div class="ratings <?php if(is_singular()) print 'hreview-aggregate'; ?>" data-post="<?php the_ID(); ?>">
  ...
  <?php if(is_singular()): // microdata only on singular pages, #ee: http//support.google.com/webmasters/bin/answer.py?hl=en&answer=146645 ?>
  <span class="item"><span class="fn"><?php the_title(); ?></span></span>
  <?php endif; ?>

  <ul <?php if(!PostRatings()->currentUserCanRate()) print 'class="rated"'; ?>  style="width:<?php print ($max_rating * 30); ?>px" title="<?php esc_attr_e($current_rating); ?>">
    <li class="rating" style="width:<?php print ($rating * 30); ?>px">
      <span class="average">
      ...
      <?php if(PostRatings()->currentUserCanRate()): // only display links if the user can rate the post ?>
        ...
        <?php for($i = 1; $i <= $max_rating; $i++): ?>
          ...
          <?php $title = apply_filters('post_ratings_control_title', sprintf(__('Give %1$d out of %2$d stars', 'post_ratings'), $i, $max_rating), $i, $max_rating); ?>
